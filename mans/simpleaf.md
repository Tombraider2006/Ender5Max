...
К сожалению  прошить датчик непосредственно с принтера не получится. так как на принтере хоть и линукс но чильно кастрированный, придется использовать костыли в виде LiveUSB с например ubuntu

На датчике должна быть установлена прошивка не старше `CARTOGRAPHER K1 5.1.0` : 

[**Инструкции по прошивке датчика**](https://pellcorp.github.io/creality-wiki/cartographer_flashing/)

[**Инструкции по прошивке датчика в среде windows. WSL Hack**](/mans/wsl.md) прошивка с помощью встроенного в windows linux

#### Ошибки при прошивке

проверить  что датчик виден в системе всегда можно командой `ls /dev/serial/by-id/*` если  система не находит ничего - проверьте соединение. Иногда путают местами провода передачи данных (зеленый и белый) их можно попробовать поменять местами

![](/images/carto_usb.png)

если все хорошо то система должна выдать результатом команды `ls /dev/serial/by-id/*` что то похожее на `/dev/serial/by-id/usb-Klipper_stm32f042x6_120031001653304B4D323431-if00` *это пример! не используйте*

В дальнейшем эта строку скрипт установки использует в файле `cartotouch.cfg`  для определения устройства и выглядеть она будет как то так:
...
<details><summary>Ручная калибровка  картографа<small><i><red> нажми чтобы раскрыть список</red></i></small></summary>
...
4. Выполните  `CARTOGRAPHER_SCAN_CALIBRATE` бумажный тест. Не используйте металлический щуп на этом этапе, он может помешать калибровке!!!
5. После окончания нажмем сохранить перегрузить или введем в панели `SAVE_CONFIG` </details>

<details><summary>Картографическое пороговое сканирование<small><i><red> нажми чтобы раскрыть список</red></i></small></summary>
...
4. Выполнить `CARTOGRAPHER_TOUCH_CALIBRATE`
...
Вот [**модель**](/mans/Ender5MaxTiltedScreenMount.stl) чтобы закрепить экран в горизонтальном положении. 
